...
import { 
  ...
  arrayUnion, updateDoc, serverTimestamp, addDoc, query, orderBy, writeBatch, deleteDoc, arrayRemove 
} from 'firebase/firestore';
import { ref, uploadString, getDownloadURL } from "firebase/storage";
import { mockProfile, mockWallet, mockChats, default as mockEvents, mockChallenges, CATEGORIES } from './data/mockData'; 
...
export const sendFriendRequest = async (sender, receiver) => {
  ...
  const outgoingRequest = { userId: receiver.id, nickname: receiver.nickname, avatar: receiver.avatar, timestamp: serverTimestamp() };
  const incomingRequest = { userId: sender.uid, nickname: sender.profile.nickname, avatar: sender.profile.avatar || `https://i.pravatar.cc/80?u=${sender.uid}`, timestamp: serverTimestamp() };
  try {
...
export const sendMessage = async (chatId, messageData) => {
  ...
  try {
    await addDoc(messagesCollectionRef, { ...messageData, timestamp: serverTimestamp() });
    return true;
  } catch (error) {
...
export const uploadImageAndGetURL = async (base64Data, userId) => {
  const imagePath = `event-images/${userId}/${Date.now()}.png`;
  const storageRef = ref(storage, imagePath);
  try {
    const snapshot = await uploadString(storageRef, base64Data, 'base64');
    return await getDownloadURL(snapshot.ref);
  } catch (error) {
    console.error("圖片上傳到 Cloud Storage 失敗:", error);
    return null;
  }
};
...
export const seedChatsToFirestore = async () => {
  ...
  for (const chatId in mockChats) {
    ...
    for (const message of chatMessages) {
      ...
      const messageWithTimestamp = { ...message, timestamp: message.timestamp || new Date() };
