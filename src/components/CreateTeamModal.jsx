...
export default function CreateTeamModal({ isOpen, onClose, friends = [], currentUser, onCreateTeam }) {
  ...
  const handleCreate = () => {
    ...
    const teamMembers = [currentUser, ...friendObjects];
    onCreateTeam && onCreateTeam(teamMembers);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      >
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="text-lg font-semibold text-slate-800">建立挑戰隊伍</h3>
        ...
        <div className="p-6">
          <label className="block text-sm font-medium text-gray-700 mb-2">選擇要邀請的好友</label>
          <div className="max-h-60 overflow-y-auto space-y-2 p-2 bg-slate-100 rounded-lg border">
            {/* 將自己顯示在最上方且不可選 */}
            {currentUser && (
              <div className="flex items-center p-2 bg-blue-50 rounded">
                <img src={currentUser.avatar} className="w-10 h-10 rounded-full mr-3" alt={currentUser.nickname}/>
                <span className="text-sm font-bold text-blue-800">{currentUser.nickname} (你)</span>
              </div>
            )}
        ...
        <div className="flex justify-end items-center p-4 border-t bg-slate-50 space-x-3">
          ...
          >
            ...
            <span>建立隊伍 ({selectedFriends.length + 1})</span>
          </button>
        </div>
      </div>
    </div>
